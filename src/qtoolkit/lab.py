...
if __name__ == '__main__':
    first_stage = PolarisationChannelMap(
        h=0,
        v=1,
        d=2,
        a=3,
    ...
    second_stage = PolarisationChannelMap(
        h=4,
        v=5,
        d=6,
        a=7,
    ...
    timetag_data = TimetagData.from_file(
        pathlib.Path.home().joinpath(
            'Projects',
            'quantum-toolkit',
            '0_dB_0_km_1_mW_5.908_sAcq.txt',
        )
    )

    processed = ProcessedTimetagData.from_timetag_data(
        timetag_data=timetag_data,
        pairs=channel_map.zz_pairs + channel_map.xx_pairs,
        coincidence_window=250,
    )

    results = BBM92Metrics(
